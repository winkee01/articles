...
对称加密的原理是通过密钥将明文加密成密文，并且再通过同一个密钥将密文解密成明文，相对于非对称加密算法速度快效率高，对于明文文本越长效率优势越大。

### 常见的对称加密算法

常见的对称加密算法有 AES、DES、3DES 等，其中 DES 由于密钥长度低容易被暴力破解，因此安全性相对较低已经不推荐使用。而 3DES 则是 DES 的升级版，进行了三次DES加密以增强安全性，但依然不如 AES，因此推荐安全性更高的 AES 加密算法。
...
另外，还有今年来适合网络的加密方式，它们的特点是速度快，性能好。最流行的是 Blowfish 和 ChaCha20。
其中 Blowfish 作为DES的替代方案，它是一种快速的加密算法，支持32位到448位的可变密钥长度。而 ChaCha20 是一种更现代的高性能流加密算法，它通常与 Poly1305 结合用于认证加密。
...
常用于密码哈希的 KDF 算法有 PBKDF2, bcrypt，scrypt，Argon2。在 2013 到 2015 年间进行的密码哈希竞赛宣布 Argon2 成为获胜者，所以目前最为推荐用于密码哈希的算法就是 Argon2。
...
Argon2 算法有三个版本：Argon2i, Argon2d 和 Argon2id，其中 Argon2id 是最优秀的版本，它是其他版本的混合体。
Argon2 的工作原理是：分配一部分内存（KDF 内存）然后用已计算的哈希值填充它直到填满。这是重复的，从它在第一个停止的内存的后续部分开始，在多个线程（KDF并行）上送代多次（KDF 迭代）。所有迭代后的结果值是您的主密钥，它充当主密码哈希的输入，用于在用户登录时验证该用户（了解更多）。
默认情况下，Bitwarden 的视线中，设置为分配 64MiB 内存，迭代 3 次，并跨 4 个线程执行此操作。这些默认值高于当前 OWASP 的推荐，所以安全程度是比较可靠的。


另外，还有一些其他常见的 KDF 算法，比如 bcrpt，scrypt 以及 yescrypt 等。安全性的比较大致如下：
...
之所以警告，是因为，在较早版本的 OpenSSL 中，OpenSSL 默认使用的是 **基于 EVP\_BytesToKey 函数的密钥派生算法**。这个算法结合了 **MD5** 散列算法来从密码生成加密密钥。EVP\_BytesToKey 函数的密钥派生过程非常简单，仅经过少量的哈希迭代（默认是一次 MD5 迭代），并且没有像 PBKDF2 那样支持复杂的多次迭代和盐值。
这种简单的派生方式使得密钥更加容易被暴力破解，尤其是使用现代硬件进行攻击时。而且 MD5 由于极其容易受到碰撞攻击（不同输入产生相同输出）
，早就被认为不再适合出现在任何实际的使用中。
...
OpenSSL 还可以生成随机密钥和初始化向量（IV），这些在某些加密模式（如AES-CBC）中是必需的。生成方法如下：
